...
// Copyright (c) 2011-2013 The GorillaBucks developers
...
const string strMessageMagic = "GorillaBucks Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // GorillaBucks: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // GorillaBucks: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // GorillaBucks: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// GorillaBucks: find block wanted by given orphan block
...
static int64 nMultiplier = 100;
    ...
    return nMultiplier * CENT;
    }

// GorillaBucks: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge, int nHeight)
{
	...
	if (nHeight < 16000) {
		...
		nSubsidy = (((nHeight % 9) + ((nCoinAge % 10000) * 33 / (365 * 33 + 8))) + 1) * nRewardCoinYear;
	}
    return nSubsidy;
}
...
// GorillaBucks: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexPrevPrev->pprev == NULL)
		...
		if (!fProofOfStake && pindexLast->nHeight >= 12864 && pindexLast->nHeight < 250000)
        return bnProofOfWorkLimit.GetCompact();


    int64 nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();

    // GorillaBucks: target change every block
    // GorillaBucks: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // GorillaBucks: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // GorillaBucks: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // GorillaBucks: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // GorillaBucks: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // GorillaBucks: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // GorillaBucks: track money supply and mint amount info
    ...
    // GorillaBucks: fees are not collected by miners as in bitcoin
    // GorillaBucks: fees are destroyed to compensate the entire network
...
// GorillaBucks: total coin age spent in transaction, in the unit of coin-days.
...
// GorillaBucks: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // GorillaBucks: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // GorillaBucks: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // GorillaBucks: record proof-of-stake hash value
    ...
    // GorillaBucks: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // GorillaBucks: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // GorillaBucks: coinbase output should be empty if proof-of-stake block
    ...
    // GorillaBucks: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // GorillaBucks: check that the block satisfies synchronized checkpoint
    ...
    // GorillaBucks: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // GorillaBucks: check proof-of-stake
    ...
    // GorillaBucks: verify hash target and signature of coinstake tx
    ...
    // GorillaBucks: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // GorillaBucks: getblocks may not obtain the ancestor block rejected
    ...
    // GorillaBucks: if responsible for sync-checkpoint send it
...
// GorillaBucks: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "GorillaBucks", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "GorillaBucks", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // GorillaBucks: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // GorillaBucks: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // GorillaBucks: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // GorillaBucks: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // GorillaBucks: should not enter safe mode for longer invalid chain
    // GorillaBucks: if sync-checkpoint is too old do not enter safe mode
    ...
    // GorillaBucks: if detected invalid checkpoint enter safe mode
    ...
    // GorillaBucks: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // GorillaBucks: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // GorillaBucks: record my external IP reported by peer
        ...
        // GorillaBucks: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // GorillaBucks: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // GorillaBucks: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // GorillaBucks: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // GorillaBucks: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // GorillaBucks: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
