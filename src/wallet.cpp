...
// GorillaBucks: optional setting to unlock wallet for block minting only;
...
// GorillaBucks: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // GorillaBucks: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // GorillaBucks: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // GorillaBucks: not avatar mode
                    {
...
// GorillaBucks: create coin stake transaction
...
// GorillaBucks: check 'spent' consistency between wallet and txindex
// GorillaBucks: fix wallet spent state according to txindex
...
// GorillaBucks: disable transaction (only for coinstake)
